...
- **`GENERATOR_VERSION` bump is easy to miss.** The `Bump policy` section above says to bump on every behavior-changing commit, but the actual failure mode is bumping late — e.g. shipping v1.5.4 with `GENERATOR_VERSION = '1.5.3'` still in `app.py`, then discovering it after the tag and release are already on the remote. Tags are immutable once published, so recovery is a fast-follow (e.g. v1.5.5 for a metadata-only bump), not a tag move. Mechanical pre-tag check: `grep "^GENERATOR_VERSION" app.py` should match the version you're about to tag. Every generated `README.md`, `CHANGELOG.md`, `example.conf`, plus the form footer and `/healthz` response carry the stamp, so a wrong bump is user-visible in a lot of places and lives forever in whatever plugins were generated during the window.
